...
class BarActivity : AppCompatActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            R.id.opcCarta -> transaction.replace(
                R.id.contenedor_fragments_bar,
                fragmentGestionarCarta
            ).commit()
            R.id.opcComandas -> transaction.replace(
                R.id.contenedor_fragments_bar,
                fragmentGestionarComandas
            ).commit()
            R.id.opcLocalizacion -> {
                val ubicacionIntent =
                    Intent(applicationContext, GestionarUbicacionActivity::class.java).apply {
                    }
